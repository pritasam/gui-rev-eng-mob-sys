...
public class Protocol implements ProtocolContext, IChangeSettingsListener {
	 ...
	 */
	public void startNormalHandling(ISessionController sessionManager,
...
//		receiverTask = receiverTaskTmp;
	...
	private PixelFormat createPixelFormat(ProtocolSettings settings) {
		...
		case ProtocolSettings.BPP_8:
			return PixelFormat.create8bppPixelFormat(serverBigEndianFlag);
	...
	public void fireChangeSettings(ProtocolSettings settings) {
		if (settings.checkAndResetPreferredEncondingChangedFlag()) {
			sendSupportedEncodingsMessage(settings);
		}
		if (pixelFormat.bitsPerPixel != settings.getBitsPerPixel() && receiverTask != null) {
