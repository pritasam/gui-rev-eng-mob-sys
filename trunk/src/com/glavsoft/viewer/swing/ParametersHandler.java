...
public class ParametersHandler {
	...
	public static void completeParserOptions(Parser parser) {
		...
		parser.addOption(Viewer.ARG_ALLOW_CLIPBOARD_TRANSFER, null, "When set to \"Yes\", transfer of clipboartd contents is allowed. " +
				"Default: \"Yes\".");
		...
		"Default: \"On\".");
		parser.addOption(Viewer.ARG_CONVERT_TO_ASCII, null, "Whether to convert keyboard input to ASCII ignoring locale. Possible values: yes/true, no/false). " +
		"Default: \"No\".");
		parser.addOption(Viewer.ARG_COLOR_DEPTH, null, "Bits per pixel color format. Possible values: 3 (for 8 colors), 6 (64 colors), 8 (256 colors), 16 (65 536 colors), 24 (16 777 216 colors), 32 (same as 24).");
		parser.addOption(Viewer.ARG_SCALING_FACTOR, null, "Scale local representation of the remote desktop. " +
				...
				"corresponds to the original framebuffer size. Only 100% is supported now.");
	}
	...
	public static void completeSettingsFromCLI(Parser parser, ConnectionParams connectionParams, ProtocolSettings settings) {
		completeSettings(parser.getValueFor(Viewer.ARG_HOST), parser.getValueFor(Viewer.ARG_PORT),
				...
				parser.getValueFor(Viewer.ARG_CONVERT_TO_ASCII), connectionParams, settings);
	...
	private static void completeSettings(String hostName, String portNumber,
			...
			String convertToAsciiParam, ConnectionParams connectionParams, ProtocolSettings settings) {
		...
		settings.setConvertToAscii(parseBooleanOrDefault(convertToAsciiParam, false));
	...
	public static void completeSettingsFromApplet(JApplet applet,
		...
		completeSettings(host,
				...
				applet.getParameter(Viewer.ARG_CONVERT_TO_ASCII),
