 ...
 * --------------------------------------edit--------------------------------------
 * @author O. Laudi
 * - replace main-method through
 * - remove "Thread"-Extension
 * - remove constructors parameters
 */ 
 
public class HoughTransform extends Thread { 
	
	public BufferedImage process(BufferedImage biSource) {
		// init
		this.width 	= biSource.getWidth(); 
        this.height = biSource.getHeight(); 
		...
		for (int j = 0; j < lines.size(); j++) { 
		    ...
		    line.draw(biOutput, Color.RED.getRGB()); 
		} 
        return biOutput;
    } 
