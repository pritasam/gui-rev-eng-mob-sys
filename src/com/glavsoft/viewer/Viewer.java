	...
	public void run() {
		...
		while (tryAgain) {
			...
			try {
				...
				clipboardController = new ClipboardControllerImpl(workingProtocol);
...
//		buttonBar.add(fileTransferButton);
	...
	protected void showReconnectDialog(String title, String message) {
		...
		reconnectDialog.setModalityType(isApplet ?
				ModalityType.APPLICATION_MODAL : ModalityType.TOOLKIT_MODAL);
			...
			if (forceConnectionDialog || wasError ||
				...
				if (null == connectionDialog) {
					connectionDialog = new ConnectionDialog(containerFrame,
							...
							optionsDialog, settings, isApplet);
				}
	...
	public void showConnectionErrorDialog(final String message) {
		JOptionPane errorPane = new JOptionPane(message.toString(), JOptionPane.ERROR_MESSAGE);
		final JDialog errorDialog = errorPane.createDialog(containerFrame, "Connection error");
		errorDialog.setModalityType(isApplet ? ModalityType.APPLICATION_MODAL : ModalityType.TOOLKIT_MODAL);
